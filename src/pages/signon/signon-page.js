...
import {StyleSheet, Text, View, TextInput, Platform,Animated,Alert} from 'react-native';
...
const toggleVerify =() => {
 ...
 const  onverifyEmail = async () => { 
  ...
  try {
    const response = await fetch('https://quranlive-api.herokuapp.com/forgotpassword',{
 ...
 if(password !== confirmPassword || password.length < 6  ){
  ...
  return;
   ...
   try {
     const response = await fetch('https://quranlive-api.herokuapp.com/verifyid',{
     //  const response = await fetch('http://192.168.11.177:3000/forgotpassword',{
        method: "put",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    ...
    {toggleForgetPassword?<Text style={{marginTop:10}} onPress={toggleVerify}> ⬅️ back to signin</Text>:null}
      ...
      <TextInput  secureTextEntry={true}onChangeText ={setPassword} placeholder =' enter new password' />
      <TextInput  secureTextEntry={true} onChangeText ={setConfirmPassword} placeholder =' re-enter new password' />
