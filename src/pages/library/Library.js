...
function Library({
  ...
  const addOnlineMushafData = async () => {
    ...
    setCopiesPending({
      ...
      transformId: currentUser[0].userid,
      title: mtitle,
      page: 1,
      cover: cover,
      source: 'test',
  ...
  return (
    <>
        ...
        {currentUser && showMes && lastMessage ? (
          currentUser[0].name === name ? null : (
            <View style={styles.topmessage}>
              <Text
                onPress={() =>
                  setLastProfile({id: lastMessage.id, name: lastMessage.name})
                }
          ...
          {userCount
            ? userCount.map((count, i) => (
                <>
                  <Text
                    key={count.count}
                    style={{left: 30, position: 'absolute', marginTop: 3}}>
                    {' '}
                    {count.count} users are praying
                  </Text>
                  <Text
                    key={i}
                    style={{
      ...
      <View style={styles.addButton}>
        <Text onPress={() => setToggleOnAdd(!toggleOnAdd)}>Add Mushaf</Text>
      </View>

      <AddCopy
        toggleOnAdd={toggleOnAdd}
        setToggleOnAdd={setToggleOnAdd}
        covers={covers}
        setCover={setCover}
        currentUser={currentUser}
        addMushafData={addMushafData}
        setTitle={setTitle}
        addOnlineMushafData={addOnlineMushafData}
      />
      ...
      {profileData ? (
        <View style={styles.profile}>
          ...
          {toggleProfiles ? (
            <Text
              style={{color: '#c2b280'}}
              onPress={() => setToggleProfiles(!toggleProfiles)}>
              Profiles -
            </Text>
          ) : (
            <Text
      ...
      {showTargets ? (
        <View style={styles.target}>
          ...
          <TextInput
            ...
            placeholder="How many pages do want to read"
            onChangeText={setTargetPages}
          />
          <TextInput
            keyboardType="numeric"
            placeholder="How many minutes do you want to pray for"
...
const mapDispatchToProps = dispatch => ({
  ...
  setCurrentOnlineMushaf: mushafId =>
    dispatch(setCurrentOnlineMushaf(mushafId)),
